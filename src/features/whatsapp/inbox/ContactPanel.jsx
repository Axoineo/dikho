...
function Row({ label, value }) {
  return (
    <div className="px-5 py-2.5">
      ...
      <div className="mt-0.5 break-words text-[13.5px] text-gray-800 dark:text-gray-100">{value || '—'}</div>
    </div>
  )
}
...
export function ContactPanel({ conversation, messages, onOpenMedia }) {
  ...
  return (
    <aside className="hidden w-[300px] shrink-0 flex-col border-l border-black/[0.07] bg-white xl:flex dark:border-white/[0.08] dark:bg-[#0f1a20]">
      <div className="flex flex-col items-center px-5 pb-5 pt-8 text-center">
        ...
        <div className="mt-3 text-[16px] font-semibold text-gray-900 dark:text-white">{displayName(conversation)}</div>
        <div className="mt-0.5 text-[13px] text-gray-500 dark:text-gray-400">+{conversation.phone}</div>
        {msLeft > 0 ? (
      ...
      {media.length > 0 && (
        <div className="min-h-0 flex-1 overflow-y-auto border-t border-black/[0.06] px-5 py-4 dark:border-white/[0.06]">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {media.slice(-12).reverse().map((m) => (
              <button key={m.id} type="button" onClick={() => onOpenMedia?.(m)} className="aspect-square overflow-hidden rounded-lg bg-black/5 dark:bg-white/10">
